function DOPU_Bscans = genDOPU_combinedBscans(DOPU_test,avgOCT,cmap_dopu_r)
    % DOPU combined B-scans with log scale avgOCT %
    % prepare matlab matrix for the Bscans in rgb

    [numPoints,numAlines,numBscans] = size(DOPU_test);
    ...
    % loop over frames
    for ii=1:size(DOPU_test,3)

        % preparing the greyscale avgOCT luminance mask
        if ii==1 || ii==size(DOPU_test,3)
